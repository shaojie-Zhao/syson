...
export default function DoDAFMatrixView() {
  ...
  const [saved] = useState(() => {
    try { const raw = localStorage.getItem(savedKey); return raw ? JSON.parse(raw) : null; } catch { return null; }
  });
  ...
  const csv = () => {
    ...
    const u = URL.createObjectURL(b); const a = document.createElement('a'); a.href = u; a.download = 'matrix.csv'; a.click(); URL.revokeObjectURL(u);
  };

  const RW = 220, CW = 140, RH = 38;

  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: '#111827', color: '#e0e0e0', fontFamily: 'system-ui' }}>
      {/* Toolbar */}
      <div style={{ display: 'flex', gap: 6, padding: '8px 12px', background: '#1e293b', borderBottom: '1px solid rgba(255,255,255,0.08)', alignItems: 'center', flexShrink: 0 }}>
        <button onClick={load} style={bs}>🔄</button>
        <button onClick={() => setTr(!tr)} style={bs}>🔀</button>
        <button onClick={csv} style={bs}>📥</button>
        <button onClick={() => { if (undo.length) { setRedo(p => [...p, [...rels]]); setRels(undo[undo.length - 1]); setUndo(p => p.slice(0, -1)); } }} disabled={!undo.length} style={{ ...bs, opacity: undo.length ? 1 : 0.4 }}>↩</button>
        <button onClick={() => { if (redo.length) { setUndo(p => [...p, [...rels]]); setRels(redo[redo.length - 1]); setRedo(p => p.slice(0, -1)); } }} disabled={!redo.length} style={{ ...bs, opacity: redo.length ? 1 : 0.4 }}>↪</button>
        <button onClick={() => setSm(s => s === 'none' ? 'id' : s === 'id' ? 'name' : 'none')} style={bs}>{sm === 'none' ? '↕' : sm === 'id' ? 'ID↓' : '名↓'}</button>
        <button onClick={() => { setHR(new Set()); setHC(new Set()); }} style={bs}>重置</button>
        <span style={{ flex: 1 }} />
        <select value={vm} onChange={e => setVm(e.target.value)} style={ss}>
          <option value="all">全部</option>
          <option value="related">有关</option>
          <option value="unrelated">缺口</option>
        </select>
        <button onClick={() => setCfg(!cfg)} style={{ ...bs, background: '#475569' }}>{cfg ? '◀' : '▶'}</button>
        <span style={{ fontSize: 11, color: '#64748b' }}>行:{sR.length} 列:{sC.length} 关系:{rels.length}</span>
      </div>
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        {/* Config Panel */}
        {cfg && (
          <div style={{ width: 220, background: '#1e293b', borderRight: '1px solid rgba(255,255,255,0.08)', padding: 12, overflowY: 'auto', flexShrink: 0, fontSize: 13 }}>
            <h4 style={{ margin: '0 0 4px', color: '#94a3b8', fontSize: 12 }}>行范围</h4>
            <div style={{ display: 'flex', gap: 4 }}>
              <input value={rowScopeName || rowScope} readOnly
                style={{ ...ss, flex: 1, cursor: 'pointer', background: rowScope ? '#0f172a' : '#111827', color: rowScope ? '#e2e8f0' : '#64748b' }}
                placeholder="拖动Package至此处"
            ...
            <div style={{ display: 'flex', gap: 4 }}>
              <input readOnly value={[...rowT].join(', ') || '全部'} style={{ ...ss, flex: 1, cursor: 'pointer' }}
                onClick={() => setTypePicker('row')} title="点击选择元类型" />
              <button onClick={() => setTypePicker('row')} style={{ ...bs, padding: '4px 8px' }}>...</button>
            </div>
            <h4 style={{ margin: '12px 0 4px', color: '#94a3b8', fontSize: 12 }}>列范围</h4>
            <div style={{ display: 'flex', gap: 4 }}>
              <input value={colScopeName || colScope} readOnly
                style={{ ...ss, flex: 1, cursor: 'pointer', background: colScope ? '#0f172a' : '#111827', color: colScope ? '#e2e8f0' : '#64748b' }}
                placeholder="拖动Package至此处"
                onDragOver={e => e.preventDefault()}
            ...
            <div style={{ display: 'flex', gap: 4 }}>
              <input readOnly value={[...colT].join(', ') || '全部'} style={{ ...ss, flex: 1, cursor: 'pointer' }}
                onClick={() => setTypePicker('col')} title="点击选择元类型" />
        ...
        <div style={{ flex: 1, overflow: 'auto' }}>
          <table style={{ borderCollapse: 'separate', borderSpacing: 0, minWidth: RW + sC.length * CW }}>
            <thead>
              <tr>
                <th style={{ ...th, width: RW, position: 'sticky', left: 0, top: 0, zIndex: 3, background: '#1e293b' }}>{tr ? '列 \\ 行' : '行 \\ 列'}</th>
                {sC.map(c => <th key={c.id} style={{ ...th, width: CW, position: 'sticky', top: 0, zIndex: 2, background: '#1e293b' }} onContextMenu={e => { e.preventDefault(); setCtm({ x: e.clientX, y: e.clientY, type: 'col', id: c.id }); }} title={c.name + (c.dodafType ? ' (' + c.dodafType + ')' : '')}>
                  <span style={{ fontSize: 10, color: '#64748b', background: 'rgba(255,255,255,0.06)', padding: '1px 5px', borderRadius: 3, marginRight: 4 }}>{c.dodafType || c.type}</span>{c.name}
                </th>)}
              </tr>
            </thead>
            <tbody>
              {sR.map((r, ri) => (
                <tr key={r.id} style={{ height: RH }}>
                  <td style={{ ...th, width: RW, position: 'sticky', left: 0, zIndex: 1, background: '#1e293b' }} onContextMenu={e => { e.preventDefault(); setCtm({ x: e.clientX, y: e.clientY, type: 'row', id: r.id }); }} title={r.name + ' (' + r.type + ')'}>
                    <span style={{ fontSize: 10, color: '#64748b', background: 'rgba(255,255,255,0.06)', padding: '1px 5px', borderRadius: 3, marginRight: 6 }}>{r.dodafType || r.type}</span>{r.name}
                  </td>
                  {sC.map((c, ci) => {
                    const crs = gcr(r.id, c.id);
                    return (
                      <td key={ri + '-' + ci} style={{ ...td, width: CW, background: crs.length ? 'rgba(59,130,246,0.08)' : 'transparent' }}
                        onClick={() => handleCellClick(r.id, c.id)} onDoubleClick={() => handleCellDblClick(r.id, c.id)}
                        title={crs.map(x => x.relationType).join(', ') || '双击新建'}>
                        {crs.map(x => <span key={x.id} style={{ color: C[x.relationType], fontWeight: 700, fontSize: 14, margin: '0 1px' }}>{S[x.relationType]}</span>)}
                      </td>
                    );
                  })}
                </tr>
      ...
      {/* Context Menu */}
      {ctm && (
        <div style={{ position: 'fixed', left: ctm.x, top: ctm.y, zIndex: 2000, background: '#1e293b', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 6, minWidth: 140, boxShadow: '0 8px 24px rgba(0,0,0,0.5)' }} onClick={() => setCtm(null)}>
          <div style={{ padding: '8px 12px', cursor: 'pointer', fontSize: 12, color: '#cbd5e1', borderBottom: '1px solid rgba(255,255,255,0.05)' }}
            onClick={() => { ctm.type === 'row' ? setHR(p => new Set([...p, ctm.id])) : setHC(p => new Set([...p, ctm.id])); setCtm(null); }}>👁️ 隐藏</div>
          <div style={{ padding: '8px 12px', cursor: 'pointer', fontSize: 12, color: '#cbd5e1' }}
            onClick={() => { setHR(new Set()); setHC(new Set()); setCtm(null); }}>🔄 恢复</div>
        </div>
      )}
      {/* Detail Modal */}
      {sel && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }} onClick={() => setSel(null)}>
          <div style={{ background: '#1e293b', padding: 20, borderRadius: 10, minWidth: 350, color: '#e0e0e0' }} onClick={e => e.stopPropagation()}>
            <h3 style={{ margin: '0 0 12px' }}>单元格关系</h3>
            {gcr(sel.row, sel.col).length === 0 ? <p style={{ color: '#64748b' }}>无关系</p> :
            ...
            <button onClick={() => setSel(null)} style={{ marginTop: 12, background: '#475569', color: '#fff', border: 'none', borderRadius: 4, padding: '6px 16px', cursor: 'pointer' }}>关闭</button>
          </div>
        </div>
      )}
      {/* Type Picker Modal */}
      {typePicker && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000, backdropFilter: 'blur(4px)' }}
          ...
          <div style={{ background: '#1e293b', padding: 24, borderRadius: 12, minWidth: 280, color: '#e0e0e0', border: '1px solid rgba(255,255,255,0.1)', boxShadow: '0 20px 60px rgba(0,0,0,0.5)' }}
            onClick={e => e.stopPropagation()}>
            <h3 style={{ margin: '0 0 16px', fontSize: 15, color: '#f1f5f9' }}>选择{typePicker === 'row' ? '行' : '列'}元类型</h3>
            {TYPES.filter(t => t !== 'ALL').map(t => {
              const sel = typePicker === 'row' ? rowT : colT;
              const setFn = typePicker === 'row' ? setRowT : setColT;
              return (
                <label key={t} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 0', cursor: 'pointer', fontSize: 13 }}>
                  <input type="checkbox" checked={sel.has(t)}
                    onChange={() => setFn(p => { const s = new Set(p); s.has(t) ? s.delete(t) : s.add(t); return s; })} />
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 16, justifyContent: 'flex-end' }}>
              <button onClick={() => setTypePicker(null)}
                style={{ background: '#475569', color: '#cbd5e1', border: 'none', padding: '6px 16px', borderRadius: 4, cursor: 'pointer' }}>确定</button>
            </div>
          </div>
        </div>
      )}
...
const bs: React.CSSProperties = { background: '#334155', color: '#cbd5e1', border: '1px solid rgba(255,255,255,0.1)', padding: '5px 10px', borderRadius: 4, cursor: 'pointer', fontSize: 12, whiteSpace: 'nowrap' };
const ss: React.CSSProperties = { width: '100%', padding: '6px 8px', borderRadius: 4, border: '1px solid rgba(255,255,255,0.1)', background: '#111827', color: '#cbd5e1', fontSize: 12 };
const th: React.CSSProperties = { padding: '6px 10px', background: '#1e293b', color: '#cbd5e1', fontSize: 12, fontWeight: 500, textAlign: 'left', borderBottom: '2px solid rgba(255,255,255,0.1)', borderRight: '1px solid rgba(255,255,255,0.06)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' };
const td: React.CSSProperties = { padding: 0, textAlign: 'center', borderBottom: '1px solid rgba(255,255,255,0.04)', borderRight: '1px solid rgba(255,255,255,0.04)', cursor: 'pointer', fontSize: 14, verticalAlign: 'middle' };
