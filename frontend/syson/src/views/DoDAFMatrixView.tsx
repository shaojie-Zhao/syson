...
export default function DoDAFMatrixView() {
  ...
  const [rowT, setRowT] = useState<Set<string>>(() =>
    saved?.rowT
      ? new Set(saved.rowT)
      : new Set(['Capability', 'OperationalNode', 'SystemNode', 'Organization', 'InformationExchange'])
  );
  ...
  const sf = (a: Elem, b: Elem) =>
    sm === 'id' ? a.id.localeCompare(b.id) : sm === 'name' ? a.name.localeCompare(b.name) : 0;
  ...
  const gcr = (ri: string, ci: string) =>
    rels.filter((r) => (r.sourceId === ri && r.targetId === ci) || (r.sourceId === ci && r.targetId === ri));
          ...
          try {
            ...
            if (elementId) {
              await fetch('/api/graphql', {
                ...
                body: JSON.stringify({
                  query:
                    'mutation deleteFromModel($input: DeleteFromModelInput!) { deleteFromModel(input: $input) { __typename ... on ErrorPayload { message } } }',
                  variables: {
                    input: { id: crypto.randomUUID?.() || 'x', editingContextId: realCtxId, elementIds: [elementId] },
                  },
                }),
              });
            }
            ...
            console.warn('[Matrix] deleteFromModel failed:', e);
          }
        }
      }
    } else if (rt) {
      ...
      try {
        ...
        if (targetObjectId && realCtxId) {
          ...
          const gqlRes = await fetch('/api/graphql', {
            ...
            body: JSON.stringify({
              query:
                'mutation createChild($input: CreateChildInput!) { createChild(input: $input) { __typename ... on CreateChildSuccessPayload { object { id } } ... on ErrorPayload { message } } }',
              variables: {
                input: {
                  id: uuid,
        ...
        console.warn('[Matrix] createChild failed:', e);
      }
    }
    load();
  };
  ...
  return (
    <div
      ...
      <div
        ...
        <span style={{ flex: 1 }} />
        <select value={vm} onChange={(e) => setVm(e.target.value)} style={{ ...ss, background: 'rgb(22, 34, 66)' }}>
          <option value="all">全部</option>
          <option value="related">有关</option>
          <option value="unrelated">缺口</option>
        </select>
        <button onClick={() => setCfg(!cfg)} style={{ ...bs, background: '#475569' }} title={cfg ? '收起' : '展开'}>
          {cfg ? '◀' : '▶'}
        </button>
                ...
                <tr key={r.id} style={{ height: RH }}>
                  ...
                  {sC.map((c, ci) => {
                    ...
                    return (
                      <td
                        key={ri + '-' + ci}
                        style={{ ...td, width: CW, background: crs.length ? 'rgba(59,130,246,0.08)' : 'transparent' }}
                        ...
                        {crs.map((x) => (
                          <span
                            key={x.id}
                            style={{ color: C[x.relationType], fontWeight: 700, fontSize: 14, margin: '0 1px' }}>
                            {S[x.relationType]}
                          </span>
                        ))}
                      </td>
