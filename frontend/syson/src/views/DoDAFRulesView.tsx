...
export default function DoDAFRulesView() {
  ...
  const thStyle: React.CSSProperties = { ...cellStyle, background: '#0f172a', fontWeight: 600, position: 'sticky', top: 0, zIndex: 1 };

  const appliedOptions = [...new Set(rules.map(r => r.applied).filter(Boolean))];

  return (
    <div id="rules-wrapper" style={{ height: '100%', display: 'flex', flexDirection: 'column', background: '#111827', color: '#e0e0e0', fontFamily: 'system-ui, sans-serif', fontSize: 13 }}>
