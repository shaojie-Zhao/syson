...
The current version used by {product} is Java 21.

You can download Java 21 JDK here: https://adoptium.net/fr/temurin/releases?version=21&os=any&arch=any

Then, you can test that your favorite shell is using Java 21 by applying this command (from anywhere):
[source,shell]
----
java --version
----
...
==== Development with Spring Tools 5 for Eclipse

If you want to develop with Eclipse, you will need Spring Tools 5 for Eclipse.

You can download Spring Tools 5 for Eclipse here: https://spring.io/tools#eclipse

Please download the latest version and try to stay up-to-date.

Once launched, please check that your Eclipse is using Java 21. Go to _Window > Preferences_ menu (or _SpringToolSuite4 > Settings_ on macOS) and then _Java > Installed JREs_.
...
The version to use is the *13.3.0* version.
...
- Services that query/mutate the model in an atomic way are located in the module `syson-sysml-metamodel-services`.
...
. Build and run the syson server locally (using docker compose as example) on `http://192.168.3.111:8080`.
. Download the documentation at the url: `http://192.168.3.111:8080/v3/api-docs/rest-apis`.
. The swagger-ui user interface is `http://192.168.3.111:8080/swagger-ui/index.html`.
