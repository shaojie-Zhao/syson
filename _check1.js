      ...
      function translateAllText(root) {
        ...
        root.querySelectorAll('*').forEach(function(el) {
          ...
          // Also translate title/tooltip attributes
      ...
      function teardownMatrix() {
        ...
        if (w) {
          ...
          try { if (w.__hiddenTable && w.__hiddenTable.isConnected) w.__hiddenTable.style.display = ''; } catch (e) {}
          w.remove();
        }
      ...
      function scanForMatrix() {
        ...
        if (document.getElementById('rules-wrapper')) return;
        if (matrixInjected) return;
      ...
      (function() {
        ...
        function show(x,y){_lastMenuX=x;_lastMenuY=y;var old=document.getElementById('dodaf-ctx');if(old) old.remove();
          var vn = getViewName();console.log('[DoDAF] show viewName=',vn);
          var e = document.getElementById('dodaf-ctx'); if(e) e.remove();
          var h = '';
          // CV-1_能力构想 specific menu
          if (vn && vn.indexOf('CV-1') >= 0) {
            ...
            var i1 = [{l:'生命线(Lifeline)',a:'node',n:'Lifeline',d:'SysMLv2EditService-PartDefinition'},{l:'组合片段(CombinedFragment)',a:'node',n:'CombinedFragment',d:'SysMLv2EditService-PartDefinition'},{l:'状态不变量(StateInvariant)',a:'node',n:'StateInvariant',d:'SysMLv2EditService-PartDefinition'},{l:'时间约束(TimeConstraint)',a:'node',n:'TimeConstraint',d:'SysMLv2EditService-PartDefinition'},{l:'持续约束(DurationConstraint)',a:'node',n:'DurationConstraint',d:'SysMLv2EditService-PartDefinition'}];
            var i2 = [{l:'包(Package)',a:'',n:'Package',d:'SysMLv2EditService-Package'},{l:'注释(Comment)',a:'',n:'Comment',d:'SysMLv2EditService-Comment'},{l:'约束(Constraint)',a:'requirement',n:'Constraint',d:'SysMLv2EditService-ConstraintUsage'}];
            var h = '<div style="width:260px;background:#1e293b;border:1px solid rgba(255,255,255,0.15);border-radius:8px;padding:8px 0;color:#e0e0e0;font:13px system-ui,sans-serif">';
            h += '<div style="padding:4px 12px;color:#64748b;font-size:11px;font-weight:600">元素</div>';
            i1.forEach(function(v){h+='<div class="di" data-a="'+v.a+'" data-n="'+v.n+'" data-d="'+v.d+'" style="padding:8px 16px;cursor:pointer">'+v.l+'</div>';});
      ...
      // ===== Custom DoDAF Rules View injection =====
      (function() {
        setTimeout(function() {
          var tab = document.querySelector('[data-testid*="tab"][data-testselected="true"]');
          if (!tab) return;
          if (tab.textContent.indexOf('OV-6a') < 0 && tab.textContent.indexOf('规则') < 0) return;
          var btn = document.createElement('button');
          btn.textContent = '＋ 添加行';
          btn.setAttribute('style', 'position:fixed;bottom:20px;right:20px;z-index:99999;background:#3b82f6;color:#fff;border:none;border-radius:8px;padding:14px 28px;cursor:pointer;font-size:16px;box-shadow:0 4px 16px rgba(0,0,0,0.4)');
          btn.onclick = function() {
            var m = window.location.pathname.match(/\/projects\/([^/]+)\/edit/);
            var ctxId = m ? m[1] : 'default-rules';
            fetch('/api/rules/default-rules/add', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ctxId: ctxId }) }).then(function(r) { return r.json(); }).then(function() { location.reload(); });
          };
