...
public class DoDAFGanttController {
    ...
    @GetMapping("/api/matrix/{repId}/target-object-id") public Map<String,String> getTargetObjectId(@PathVariable String repId, @RequestParam String ctxId, @RequestParam(defaultValue="") String matrixRepId, @RequestParam(defaultValue="true") boolean tableOnly) { String effectiveRepId = matrixRepId.isBlank() ? repId : matrixRepId; String toi = s.getTargetObjectId(ctxId, effectiveRepId, tableOnly); String ec = s.getEditingContextId(ctxId); String vn = s.getViewName(ctxId, toi); Map<String,String> r = new java.util.HashMap<>(); r.put("targetObjectId", toi == null ? "" : toi); r.put("editingContextId", ec == null ? "" : ec); r.put("viewName", vn == null ? "" : vn); return r; }
